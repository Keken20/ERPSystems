...
$(document).ready(function () {
    ...
    $('#dropdown1').on('click', function () { //Approve Status
        if (!firstButtonClicked) {
            ...
            $.ajax({
                type: 'POST',
                url: '/AdminPage/UpdateStatus',
                data: { reqid: ReqId, decision: Decision },
                dataType: 'json',
    ...
    $('#dropdown2').on('click', function () { //Disapprove Status
        ...
        if (!firstButtonClicked) {
            ...
            console.log(ReqId);
            $.ajax({
                url: "/AdminPage/UpdateStatus", // Replace with your actual server-side endpoint
                method: "POST", // or "GET" depending on your requirements
                data: { reqid: ReqId, decision: Decision }, // Data to send to the server
                success: function (response) {
                    // Handle the response from the server, e.g., update the UI
                    alert(response.message)
                    $('.invoice-wrapper.show').hide();
                    window.location.href = '/AdminPage/RequestList';
                },
                error: function (error) {
                    console.error(error);
                    alert(error);
    ...
    $('#Backbutton').on('click', function () { //Disapprove Status
        $('.invoice-wrapper.show').hide();
        window.location.href = '/AdminPage/RequestList';
    });
